...
trait Pipeline {
  type Tp <: Pipeline
  val plugins = ArrayBuffer[Plugin[Tp]]()
...
//  val services = ArrayBuffer[Any]()
  ...
  def build(): Unit = {
    plugins.foreach(_.pipeline = this.asInstanceOf[Tp])
    plugins.foreach(_.setup(this.asInstanceOf[Tp]))

    plugins.foreach { p =>
      p.parentScope =
        Component.current.dslBody //Put the given plugin as a child of the current component
      p.reflectNames()
    }

    //Build plugins
    plugins.foreach(_.build(this.asInstanceOf[Tp]))
    ...
    for (stageIndex <- 0 until stages.length; stage = stages(stageIndex)) {
      stage.inserts.keysIterator.foreach(signal =>
        inputOutputKeys
          .getOrElseUpdate(signal, new KeyInfo)
          .setInsertStageId(stageIndex)
      )
    ...
    for (stageIndex <- 0 until stages.length; stage = stages(stageIndex)) {
      stage.arbitration.isFlushed := stages
        ...
        .orR || stages.drop(stageIndex).map(_.arbitration.flushIt).orR
      if (!unremovableStages.contains(stage))
        stage.arbitration.removeIt setWhen stage.arbitration.isFlushed
      else
        assert(
          stage.arbitration.removeIt === False,
          "removeIt should never be asserted on this stage"
        )
